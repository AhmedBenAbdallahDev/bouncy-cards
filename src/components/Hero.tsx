...
export default function Hero() {
  ...
  useAnimationFrame(() => {
    ...
    const pointerSpeed = Math.hypot(mvX, mvY);
    ...
    cards.current.forEach(c => {
      ...
      if (s.cx !== 0 && s.cy !== 0 && mouse.current.active) {
        ...
        if (b < 1) {
          const falloff = 1 - b;
          const basePush = Math.pow(falloff, 6) * 0.38;
          const flick = Math.max(0, Math.min((pointerSpeed - 18) / 90, 1));
          const throwPush = Math.pow(falloff, 3) * flick * 0.75;

          s.vx += mvX * (basePush + throwPush);
          s.vy += mvY * (basePush + throwPush);

          const speed = Math.hypot(s.vx, s.vy);
          if (speed > 52) {
            const scale = 52 / speed;
            s.vx *= scale;
            s.vy *= scale;
          }
        }
  ...
  return (
    <section className="relative w-full min-h-[100dvh] flex items-center justify-center bg-[#f3f1eb] text-[#111] overflow-hidden px-4 md:px-8 py-10 md:py-14">
      <div className="w-full max-w-7xl flex flex-col justify-between min-h-[calc(100dvh-5rem)]">
        <div className="pt-4 md:pt-8 text-center">
          <h1 className="font-display font-black uppercase flex flex-col gap-2 md:block md:space-x-8" style={{ fontSize: 'clamp(2.5rem, 8vw, 7rem)', lineHeight: '0.85', letterSpacing: '-0.02em' }}>
          ...
          <motion.h2
            className="mt-4 uppercase font-sans font-semibold tracking-[0.2em] text-[0.75rem] md:text-[1rem] opacity-70 mx-auto text-center max-w-2xl"
        ...
        <div className="flex justify-center items-center pointer-events-none relative z-10 w-full touch-none py-6 md:py-0">
        {cards.current.map((card, i) => (
           <div 
             key={card.id} 
             ref={el => { card.el = el; }}
             className={`relative aspect-square w-[clamp(6rem,18vw,15rem)] md:w-[clamp(8rem,15vw,17rem)] flex-shrink-0 grid place-items-center rounded-[1rem] md:rounded-[1.3rem] ${i !== 3 ? 'mr-[-4%] md:mr-[-6%]' : ''}`}
             style={{ 
               opacity: mounted ? 1 : 0, 
        ...
        <div className="pb-4 md:pb-8 text-center">
          <p className="mx-auto max-w-2xl font-sans text-sm md:text-base leading-relaxed opacity-70">
            A centered, screen-filling stack built so the motion reads first and the copy frames it, not fights it.
          </p>
        </div>
      </div>
    </section>
